...
export function calculateTranslationCopies(originalPoints, dragVector, n) {
    const copies = [];
    if (n <= 0) return copies;
    for (let i = 1; i <= n; i++) {
        const stepVector = {
            x: dragVector.x * (i / n),
            y: dragVector.y * (i / n)
        ...
        copies.push(...newPoints);
    }
    return copies;
}

export function calculateRotationCopies(originalPoints, center, angle, n) {
    const copies = [];
    if (n <= 0) return copies;
    const angleStep = angle / n;
    for (let i = 1; i <= n; i++) {
...
export function calculateScaleCopies(originalPoints, center, scale, n) {
    const copies = [];
    if (n <= 0) return copies;
    const scaleStep = (scale - 1) / n;
    for (let i = 1; i <= n; i++) {
        const currentScale = 1 + (scaleStep * i);
        const newPoints = originalPoints.map(p => {
            const translatedX = p.x - center.x;
            const translatedY = p.y - center.y;
            const scaledX = translatedX * currentScale;
            const scaledY = translatedY * currentScale;
            return {
                ...p,
                id: generateUniqueId(),
                x: scaledX + center.x,
...
export function calculateMirrorCopy(originalPoints, p1, p2) {
    const copies = [];
    const dx = p2.x - p1.x;
    const dy = p2.y - p1.y;
    ...
    originalPoints.forEach(p => {
        const translatedX = p.x - p1.x;
        const translatedY = p.y - p1.y;
        const mirroredX = translatedX * a + translatedY * b + p1.x;
        const mirroredY = translatedX * b - translatedY * a + p1.y;
        copies.push({
            ...p,
            id: generateUniqueId(),
            x: mirroredX,
